...
<div class="post-list">
    <div class="post-list-title">
        <h4>{{ $post['title'] }}</h4>
        <p><small>{{ Format::formats_date($post['publish_date']) }}. posted on: <a href="{{ URL::to('category/'.$post->ofCategory[0]->category->slug) }}">{{ $post->ofCategory[0]->category->name }}</a></small></p>
        <hr>
    </div>
